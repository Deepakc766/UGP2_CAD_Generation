...
GEN_DEVICE = 0  # -1 for CPU, >=0 for GPU indexlit 
...
def pdf_bytes_to_images(pdf_bytes: bytes, dpi: int = 300) -> list[Image.Image]:
    ...
    for image in pil_images:
        ...
        # Denoise using fast Non-local Means
...
# LangGraph-style Orchestrator (lightweight)
...
def node_text_chunking(raw_text: str) -> List[Document]:
    ...
    separator="\n\n",   # split by paragraph
    chunk_size=1000,    # bigger chunks for contracts
    chunk_overlap=200
    )
...
# Make sure EMBEDDING_MODEL is defined globally
...
EMBEDDING_MODEL_NAME = "nlpaueb/legal-bert-base-uncased"#using legal bert for embedding vector


def node_embed_and_index(docs: list[Document], persist_dir: str = FAISS_INDEX_DIR) -> FAISS:
    st.info("Embedding text and building FAISS vector store...")
    ...
    if os.path.exists(index_path):
        ...
        vectordb = FAISS.load_local(
            persist_dir, embedder, allow_dangerous_deserialization=True
        )
...
def build_generation_pipeline():
    ...
    pipe = pipeline("text2text-generation", model=model, tokenizer=tokenizer, device=GEN_DEVICE, max_length=1024)
...
def node_generate_cad(rag_chain, output_name: str = "generated_CAD.docx"):
    ...
    # -----------------------------
    # 2) Important Submittals → Stage/Document/Timeline table
    # -----------------------------
